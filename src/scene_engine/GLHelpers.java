...
public class GLHelpers {
    ...
    public static void drawImageFromTextureName(int name, int x, int y, int w, int h) {
        ...
        // CRITICAL (maybe not) - Reset the color (appears be be per-tile layer basis, handled there)
...
//        GL11.glColor4f(r, g, b, 1.0F);
    ...
    public static void pushNewColor(Color color) {
        lastColor = currentColor;
        currentColor = color;
        GL11.glColor4f(color.getRed() / 255.0F, color.getGreen() / 255.0F, color.getBlue() / 255.0F, color.getAlpha() / 255.0F);
    }
    
    public static void popPreviousColor() {
        currentColor = lastColor;
        pushNewColor(currentColor);
    }
