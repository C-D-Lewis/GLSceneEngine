...
public class FontRenderer {
    ...
    public static Dimension getTextSize(String content, int width, int fontSize) {
        ...
        for(int i = 0; i < content.length(); i++) {
            ...
            if(renderPoint.x + fontSize > width) {
                ...
                if(maxWidth == 0 || maxWidth < renderPoint.x + fontSize) maxWidth = renderPoint.x - glyphGap;
                renderPoint.x = 0;
    ...
    public static void drawString(String content, Rectangle bounds, int fontSize, int hAlign, int vAlign) {
        ...
        if(!content.contains(" ")) lines.add(words[0]);  // Single word
        else {
            // For every word...
            int lineLength = 0;
            String line = "";
            for(String word : words) {
                ...
                if(word.equals(words[words.length - 1])) lines.add(line);  // If the last word, finish the line
            }
        }
        ...
        for(String l : lines) {
            ...
            if(Config.DRAW_TEXT_BOUNDS) {
                GLHelpers.pushNewColor(Color.PINK);
            ...
            for(char c : chars) {
                ...
                int name = sheet.bindTileTextureToGPU(sheetPoint);
